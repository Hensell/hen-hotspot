...
public sealed record GuardCommand(string Operation, string[] ApprovedMacs, string Language = "es");
...
public static class GuardWire
{
    ...
    public static async Task SendAsync<T>(Stream stream, T value, CancellationToken token)
    {
        ...
        if (data.Length > MaxLength) throw new InvalidDataException(L10n.T("TheAccessControlMessageExceedsItsSizeLimit"));
    ...
    public static async Task<T> ReceiveAsync<T>(Stream stream, CancellationToken token)
    {
        ...
        if (length is <= 0 or > MaxLength) throw new InvalidDataException(L10n.T("InvalidAccessMessage"));
        byte[] data = new byte[length]; await stream.ReadExactlyAsync(data, token);
        return JsonSerializer.Deserialize<T>(data) ?? throw new InvalidDataException(L10n.T("EmptyAccessMessage"));
    }
}
...
public sealed class DeviceGuardClient : IAsyncDisposable
{
    ...
    {
        if (pipe is not null) throw new InvalidOperationException(L10n.T("AccessControlHasAlreadyStarted"));
        ...
        try
        {
            ...
            helper = await Task.Run(() => Process.Start(start)) ?? throw new InvalidOperationException(L10n.T("CouldNotOpenTheAdministratorPermissionPrompt"));
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(40));
            await pipe.WaitForConnectionAsync(timeout.Token);
            if (!GetNamedPipeClientProcessId(pipe.SafePipeHandle, out uint pid) || pid != helper.Id)
                throw new InvalidOperationException(L10n.T("CouldNotVerifyTheAccessControlProcess"));
            await UpdateApprovedAsync(approved);
            if (!Active) throw new InvalidOperationException(Snapshot.Error ?? L10n.T("AccessControlDidNotStart"));
    ...
    public async Task UpdateApprovedAsync(IEnumerable<string> macs)
    {
        ...
        if (!Snapshot.Active) throw new InvalidOperationException(Snapshot.Error ?? L10n.T("AccessControlStopped"));
    }
    private static string[] Normalize(IEnumerable<string> values)
    {
        var result = values.Select(DeviceAccessStore.NormalizeMac).Distinct().Order().ToArray();
        if (result.Length > 128) throw new ArgumentException(L10n.T("UpTo128AuthorizedDevices"));
        return result;
    }
    private async Task ExchangeAsync(GuardCommand command)
    {
        ...
        try
        {
            ...
            await GuardWire.SendAsync(pipe ?? throw new ObjectDisposedException(nameof(DeviceGuardClient)), command with { Language = L10n.LanguageCode }, timeout.Token);
    ...
    private async Task HeartbeatAsync()
    {
        ...
        {
            if (releasing) return;
            Volatile.Write(ref snapshot, Snapshot with { Active = false, Error = L10n.T("AccessControlConnectionLost") + ex.Message });
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (command.Operation != "apply" || command.ApprovedMacs is null || command.ApprovedMacs.Length > 128)
                    throw new InvalidDataException(L10n.T("InvalidAccessRequest"));
                var status = await new HotspotService().ReadAsync();
                if (status.State != "On" || status.Error is not null) throw new InvalidOperationException(L10n.T("TheHotspotMustBeOnAndAvailable"));
                var network = DeviceGuardPolicy.FindInterface();
                if (previous is not null && previous.Network != network)
                    throw new InvalidOperationException(L10n.T("TheHotspotInterfaceChangedEnableAccessControlAgain"));
